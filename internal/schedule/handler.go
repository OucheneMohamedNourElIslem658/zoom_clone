...
func (sh *ScheduleHandler) SearchMeetings(ctx context.Context, req *pb.SearchMeetingsRequest) (*pb.SearchMeetingsResponse, error) {
	...
	meetings, err := sh.scheduleRepo.GetAllMeetings(userID, req)
	if err != nil {
		return nil, err
	}

	meetingsResponse := make([]*pb.Meeting, 0, len(meetings))
	for _, meeting := range meetings {
		meetingsResponse = append(meetingsResponse, &pb.Meeting{
			Id:          uint32(meeting.ID),
			Title:       meeting.Title,
			Description: meeting.Description,
			StartTime:   timestamppb.New(meeting.StartTime),
			IsCancelled: meeting.IsCancelled,
			Type:        pb.MeetingType(pb.MeetingType_value[string(meeting.Type)]),
		})
	}

	return &pb.SearchMeetingsResponse{
